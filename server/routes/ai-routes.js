...
// LEGACY /clients/chat + /clients/chat/continue — removed. /86/chat
// (and /86/chat/continue) handle every former HR surface. Stubs
// return 410 in case an old client cache still tries the legacy paths.
router.post('/clients/chat', requireAuth, (req, res) => {
  res.status(410).json({ error: 'Removed. Use /api/ai/86/chat with entity_type=client.' });
});
router.post('/clients/chat/continue', requireAuth, (req, res) => {
  res.status(410).json({ error: 'Removed. Use /api/ai/86/chat/continue.' });
});
...
// (and /86/chat/continue) handle every former CoS surface; cra and
// staff Anthropic-side agents were archived.
router.post('/staff/chat', requireAuth, (req, res) => {
  res.status(410).json({ error: 'Removed. Use /api/ai/86/chat with entity_type=staff or admin.' });
});
router.post('/staff/chat/continue', requireAuth, (req, res) => {
  res.status(410).json({ error: 'Removed. Use /api/ai/86/chat/continue.' });
});
