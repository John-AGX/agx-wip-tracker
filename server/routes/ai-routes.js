...
const SECTION_DEFAULTS = {
  ...
  ag_tools: {
    ...
    body: '# Your propose_* tools (every one is approval-required)\nTool names say "section"; the UI calls them "subgroups" — same thing.\n  • propose_add_line_item / propose_update_line_item / propose_delete_line_item — line CRUD\n  • propose_add_section / propose_update_section / propose_delete_section — subgroup CRUD. Omit markup_pct (or 0) — markup is set per estimate by the user.\n  • propose_update_scope — set/append the ACTIVE group\'s scope.\n  • propose_switch_active_group — switch which group is active BEFORE proposing lines/scope. Use when the user pivots ("now the roof") instead of slotting under the wrong group.\n  • propose_add_group — new group, auto-seeds the four standard subgroups. copy_from_active=true clones the active group\'s lines.\n  • propose_rename_group / propose_delete_group / propose_toggle_group_include — group management + Good/Better/Best toggle.\n  • propose_link_to_client / propose_link_to_lead / propose_update_estimate_field — link an unlinked estimate (use read_clients / read_leads first) + update top-level metadata.\n  • propose_bulk_update_lines / propose_bulk_delete_lines — same field across N lines in one approval card.\nLine + subgroup ids come from the estimate context below. Fire multiple parallel proposals when batching — single bulk Approve-all card.'
  },
  ag_slotting: {
    agent: 'job',
  ...
  ag_auto_reads: {
    ...
    body: '# Auto-tier read tools (no approval, run as inline chips)\n  • `read_materials(q?, subgroup?, category?, limit?)` — AGX catalog: description, SKU, unit, last/avg/min/max prices, last-seen, count. Call BEFORE quoting materials. Tight keyword ("5/4 PT decking", "Hardie lap 8.25").\n  • `read_purchase_history(material_id?, q?, days?, job_name?, limit?)` — receipt-level rows for a SKU. Trends + per-job usage.\n  • `read_subs(q?, trade?, status?, with_expiring_certs?, limit?)` — sub directory with cert expiry. with_expiring_certs=true for pre-bid audit.\n  • `read_lead_pipeline(...)` — leads + status rollup. Sibling-context / pipeline-shape questions.\n  • `read_clients(q?, limit?)` — directory lookup. Call BEFORE propose_link_to_client when an estimate is unlinked.\n  • `read_leads(q?, status?, limit?)` — direct lead lookup. Call BEFORE propose_link_to_lead.\n  • `read_past_estimate_lines(q, days?, limit?)` — pricing benchmark across past AGX estimates. Returns median/range. Call BEFORE quoting a labor/sub line. (Materials come from read_materials — real receipts.) 0 matches → mark "first-time line, no AGX history" + quote a defensible Central-FL number.\n  • `read_past_estimates(...)` — past estimate lookup by title + client. Find a comparable to model on.\n  • `read_active_lines({estimate_id, section_id?, limit?})` — full line detail when the per-turn context shows compact roll-ups (>12 cost lines). Use to get line_ids before update/delete.\n  • `read_attachment_text({attachment_id, max_chars?})` — full body of an attached PDF/XLSX/DOCX/CSV/TXT. The manifest shows only a 200-char preview.\n  • `load_skill_pack({name})` — pull the FULL body of a manifest-listed skill pack on-demand. Don\'t pre-load; load when starting a kind of work that maps to it.\n  • `self_diagnose({window_minutes?, estimate_id?})` — introspect your own recent tool_uses. Call when the user says "you didn\'t do X" / "still nothing".\nCap auto-tier reads at ~4 per turn for normal estimates.\n**Hard rule — no read loops.** If `read_materials` comes back empty/sparse, DO NOT retry with narrower keywords. BROADEN to `web_search` (catalog only has SKUs AGX has actually bought — first-time line items need a web lookup). After web also fails, quote a defensible Central-FL estimate from trade knowledge and mark the rationale. After ~3 read_materials calls in a row without either a web_search OR a propose_*, the panel hard-stops the loop.'
  },
  ag_web_research: {
    agent: 'job',
