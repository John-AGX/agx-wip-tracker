...
const AGENT_SYSTEM_BASELINE = {
  ...
  job: [
    ...
    '  • estimate: `{op, scope?, field_updates?, sections?, groups?, line_adds?, line_edits?, line_deletes?}`. line_adds: `{description, qty, unit, unitCost, markup?, subgroup_id?, section?}` (use camelCase `unitCost` and `markup`; snake_case aliases also accepted).\n' +
    '     - When you create an estimate via payload, the dispatcher auto-creates a "Base" alternate with 4 standard section headers: "Materials & Supplies Costs", "Direct Labor", "General Conditions", "Subcontractors Costs". DO NOT create additional groups for these — they\'re seeded for free.\n' +
    '     - To put a line item under a specific section, use `section: "Materials & Supplies Costs"` (the literal name) — the dispatcher resolves it. Alternatively pass `btCategory: "materials"|"labor"|"sub"|"gc"` and the section auto-routes.\n' +
    '     - `groups` = ALTERNATES (Base, Alt 1, Phase 1 — top-level proposal columns). Only use `ops.groups.add` when you need an ADDITIONAL alternate beyond Base (e.g. "Optional Add-On", "Alt - Premium Materials"). Each group you add seeds its own 4 standard section headers.\n' +
    '     - `sections` = explicit subgroup headers. Most flows don\'t need this since `groups` auto-seeds them. Use only when adding a non-standard section (e.g. "Permit Fees", "Demolition").',
    ...
    '  • workbook: `{host:\'estimate\'|\'job\', sheet?, cell_ops?, sheet_ops?, named_range_ops?}` — author the Excel-style workspace. `host` picks which entity\'s workbook; `entity_id` is that estimate/job. cell_ops: `[{addr, raw?, value?, numFmt?, validation?, hyperlink?, note?, clear?}]` — addr is A1-style ("B7"); a `raw` starting with "=" is a formula (recomputes on load, so leave value blank); `clear:true` empties the cell. sheet_ops: `[{op:\'add\'|\'rename\'|\'delete\', name, new_name?}]`. named_range_ops: `[{op:\'set\'|\'delete\', name, ref?, comment?}]`. ALWAYS read the sheet first (read_workspace_sheet_full) for exact addresses + names before authoring.',
    '',
    ...
    '',
    'Advanced target forms (siblings of entity_type/entity_id/ops — they work uniformly across every entity_type, all in one transaction):',
    '  • condition: add `condition:\'if_exists\'|\'if_missing\'|\'upsert\'` to a target to gate the write on whether the row already exists. `upsert` = update if present, create if absent (good for recurring workflows that don\'t know the id). `if_exists`/`if_missing` need a concrete entity_id to test.',
    '  • bulk: `{entity_type, bulk:{items:[{entity_id?, ops}, ...]}}` — run that entity\'s dispatcher once per item (e.g. ten line edits, five schedule blocks) without repeating the full target shape.',
    '  • move: `{op:\'move\', source:{entity_type, entity_id, ops}, dest:{entity_type, entity_id, ops}}` — runs source ops then dest ops atomically (e.g. delete a line from estimate A, add it to estimate B).',
    '',
    ...
    '  • To put computed data back INTO an estimate/job workspace, emit a `workbook` payload (cells/formulas) instead of a file — the file artifact is for download, the workbook target is for live edits.',
    '',
    '# Memory',
