...
const AGENT_SYSTEM_BASELINE = {
  ...
  job: [
    ...
    'CHANGE ORDERS: the job-context block above names them ("CO-3 — income $X, cost $Y") but carries no ids and no lines, so it is never enough to write from. Call `read_change_orders` FIRST — it returns the `co_id` and every line\'s `line_id`, unit cost, promised Unit Sell and markup — then hand the Scribe those exact ids. To change ONE line, ask for a `line_edits` op naming its `line_id`; NEVER ask for a full `fields.lines` replacement, which drops every line you did not list and reports success while doing it. `unitCost` is what the work COSTS; `unitSell` is the price promised to the owner, and a line that has one prices at that price no matter what the cost is — so when a change order shows no profit, the repair is usually to put the quoted number in `unitSell` and the real number in `unitCost`. An approved (locked) or applied change order cannot be edited by anyone, including you: say so rather than trying.',
