...
router.post('/skills/sync-all-to-anthropic', requireAuth, requireSystemAdmin, async (req, res) => {
  try {
    ...
    for (let i = 0; i < skills.length; i++) {
      const pack = skills[i];
      // Delegates to the same create -> re-point -> retire path the per-pack
      // route uses. This loop used to create-only and skip anything with an
      // id, so an edited playbook could never ship through here — while it
      // reported 'already_synced', which was true only of the id.
      const currentHash = skillBodyHash(buildSkillMarkdown(pack));
      if (pack.anthropic_skill_id && pack.synced_hash === currentHash) {
        summary.push({ idx: i, name: pack.name, status: 'already_synced',
                       anthropic_skill_id: pack.anthropic_skill_id });
        continue;
      }
      const isUpdate = !!pack.anthropic_skill_id;
      try {
        const pushed = await pushPackToAnthropic(anthropic, pack);
        // Persist the id + fingerprint back onto the local pack. Mutating
        // skills[i] then writing the whole row preserves admin edits
        // on other packs (no race because admin is single-user).
        skills[i] = Object.assign({}, pack, {
          anthropic_skill_id: pushed.id,
          synced_hash: pushed.hash
        });
        summary.push({
          idx: i, name: pack.name,
          status: isUpdate ? 'updated' : 'synced',
          anthropic_skill_id: pushed.id,
          replaced: pushed.replaced,
    ...
    // landed.
    const newSyncs = summary.filter(s => s.status === 'synced' || s.status === 'updated').length;
    if (newSyncs > 0) {
      await pool.query(
        `INSERT INTO app_settings (key, value, updated_at)
         VALUES ('agent_skills', $1::jsonb, NOW())
...
router.post('/skills/:idx/sync-to-anthropic', requireAuth, requireSystemAdmin, async (req, res) => {
  try {
    ...
    // Re-syncing an ALREADY-synced pack used to be impossible. This route
    ...
    // mirror-image bug: it SKIPPED anything with an id, so it could only
    // create, never update. Between them, content edits never propagated.
    //
    // Both now call pushPackToAnthropic, which owns the ordering (upload →
    // re-point → retire). The paths diverging is what let edits go unshipped.
    const pushed = await pushPackToAnthropic(anthropic, pack);

    // Persist the swap, including the fingerprint of what was uploaded
    // so sync-all can tell "matches live" from merely "has an id".
    skills[idx] = Object.assign({}, pack, {
      anthropic_skill_id: pushed.id,
      synced_hash: pushed.hash
    });
    await pool.query(
    ...
    // Retiring the old skill already happened inside pushPackToAnthropic
    ...
    res.json({
      ...
      replaced: pushed.replaced,
      agent_rows_repointed: pushed.repointed,
      old_deleted: pushed.oldDeleted,
      old_delete_error: pushed.oldDeleteError,
      note: pushed.replaced ? 'Run managed sync-all + start a new chat for the agent to pick this up.' : undefined
    });
  } catch (e) {
...
const LINK_CONVENTION = [
  ...
  '- Plain pages: /jobs /estimates /leads /clients /subs /schedule /insights /my-day /my-tasks /messages /email-hub /cost-inbox /invoices /service-tickets /projects /plans /assemblies /files',
...
// so it could not have done it — and instead of saying so it invented a
// confirmation. A silent no-op that REPORTS SUCCESS is worse than an error,
// because nothing tells the user to go check.
const WRITE_HONESTY = [
  '# Never claim a write you did not make',
  'Describe a record as created / updated / queued / applied ONLY if you actually called the write tool in THIS turn and saw its result. If you did not call it, do not say it is done, queued, in progress, or "applying shortly". A false confirmation is worse than an error: an error makes the user look, a fake success makes them stop looking.',
  'If you have no tool for what was asked, say so plainly in one sentence and name the manual path instead of improvising. Example: there is no estimate→job conversion tool — tell the user to open the estimate and use the "Create Job" button.',
  'Never re-emit a write to "make sure it landed". Writes are not idempotent: a second emit creates a SECOND record, it does not retry the first. If a write tool tells you the change was already submitted, relay that and stop.',
  // 2026-09-21: every write 86 drafted for the Fairways emails failed at the
  // Scribe, and 86 then told John his tasks were "waiting on the approval
  // card". The Scribe had refused the tasks because 86 handed over a NAME and
  // an email where a user id goes (read_users did not even print ids), and
  // the work order because 86 gave every building an assignee. 86 could not
  // see either refusal: the notice is posted to the thread, not to its
  // session. These two rules, with <recent_writes> in the turn context and
  // the ids read_users now prints, close all three.
  '# Assignees are user ids — never a name, never a guess',
  "When a task or a work order names who is doing it, the Scribe needs that person's NUMERIC user id. It cannot look people up, and a name or an email where the id goes is refused and nothing is saved. So find the person first with search_entities entity_type `user` — every row shows `user #N` — and write `assignee_user_id N` into the instruction. For the person you are talking to, use the id in <acting_user>. If a name matches more than one user (two people can share a name), ASK which one, showing their emails; never pick one yourself. A child task on a work order — a building on its punch list — is never assigned to anyone: the person goes on the work order's own assignee, once.",
  '# Say what actually happened to a write',
...
const AGENT_SYSTEM_BASELINE = {
  ...
  //
  // What's left: identity + the two universal reads + scribe_write (86 does
  // not hold emit_payload_file — the Scribe does, and the per-entity_type ops
  // vocabulary lives in the Scribe's baseline below). The baseline was cut to
  // under 200 words at the time (was ~700); it has grown since.
  job: [
    ...
    'Supported entity_types: job, estimate, client, lead, user, pipeline, material, sub, business_card, task, wip, service_ticket (read_entity only — a work order on a job or lead, by its id only). Resolve real entity_ids via these reads BEFORE emitting a payload.',
    'You can ALSO read (inline tools): the user\'s calendar (read_calendar_events), reminders (read_reminders), production schedule (read_schedule_blocks), projects (read_projects), the PHOTOS ON ANY RECORD THAT HOLDS THEM — a LEAD, a JOB, a PROJECT or a TASK — with their descriptions, tags and GPS (read_project_photos: pass entity_type + entity_id, e.g. {entity_type:\'lead\', entity_id:\'lead_…\'}; the older project_id spelling still means entity_type:\'project\'. Use it for "what did we shoot today", "which photos have no description/tags", and to gather attachment ids before captioning, tagging or building a report from a photo set. You DO have a way to see and caption the images on a LEAD — do not tell the user otherwise. To WRITE those descriptions or tags, hand the Scribe an `attachment` target with `photo_updates: [{attachment_id, caption?, tags?}]` — one target for the whole set, addressed by the attachment ids this read prints in square brackets), purchase orders (read_purchase_orders — read-only), change orders WITH THEIR LINE ITEMS (read_change_orders — read-only), and nearby jobs/leads by location (find_entities_near). Use these for "what\'s on my calendar / day", "what projects / POs do we have", "what\'s near me".',
    'CHANGE ORDERS: your per-turn job context does NOT list them, and carries no money of any kind — no WIP, no contract or change-order totals, no phase or building structure, no invoices, no POs, no QuickBooks cost. What it does carry is: the job\'s header fields (id and title always; job number, client, community, address, type, status and target margin when the job has them), an attachment roll-up (documents by filename, photos by attachment id) and the job notes. That is the whole of it, so you will not see a change order until you read one. Call `read_change_orders` FIRST — it returns the `co_id` and every line\'s `line_id`, unit cost, promised Unit Sell and markup — then hand the Scribe those exact ids. To change ONE line, ask for a `line_edits` op naming its `line_id`; NEVER ask for a full `fields.lines` replacement, which drops every line you did not list and reports success while doing it. `unitCost` is what the work COSTS; `unitSell` is the price promised to the owner, and a line that has one prices at that price no matter what the cost is — so when a change order shows no profit, the repair is usually to put the quoted number in `unitSell` and the real number in `unitCost`. A change order at `pending` is PENDING APPROVAL — written up and sent to the owner, nobody has signed it. It is still fully editable and it is worth exactly $0: like a draft, it is outside every contract, WIP, backlog and pay-application total, so moving a change order to pending does NOT change any money on the job. An approved (locked) or applied change order cannot be edited by anyone, including you: say so rather than trying.',
    ...
    'Estimates AND jobs each carry a multi-sheet workbook — formulas, number formats, dropdown validations, named ranges, hyperlinks, cell notes. On an ESTIMATE the per-turn "# Workspace sheets" index lists what exists; on a JOB there is no such index in your context, so ask for the sheet by name or read the job first. To pull a full sheet call `read_workspace_sheet_full({sheet_name})` (read-only, auto-applies, no approval) — sheet_name must match the index verbatim. A job\'s workbook is INHERITED from its parent estimate at lead→job conversion, then diverges; read the job\'s own copy for live job data, not the estimate\'s. (Skip "QB Costs <date>" / "Detailed Costs" sheets — use read_qb_cost_lines for QuickBooks data.)',
    '',
    ...
    'You do NOT author writes yourself. To change anything — field updates, line-item edits, phase changes, change orders, lead/client creates, schedule blocks, reports, skill/field-tool config, etc. — call `scribe_write` with a plain-words `instruction`. The Scribe turns it into the actual change and dry-runs it. FAST LANE: for a small change — plain fields on ONE lead/client/job/service ticket, or ONE new to-do/task/reminder/calendar event — call `quick_write` instead; it is drafted in this turn. APPROVAL: every draft reaches the user as ONE line with an Approve button. Do not ask "shall I?" first — a yes before that line exists approves nothing. When the user says yes AFTER the line appears, call `approve_pending_write` (no input) and reply with only the receipt line it returns; if it refuses, relay that in one line. Deletes, status changes, money and % complete always need a tap.',
    ...
    'SERVICE TICKETS CANNOT BE LISTED: no job read and no lead read lists a record\'s service tickets, and none is searchable — a ticket can be read only by its id, with read_entity entity_type `service_ticket`. So NEVER say a job or lead has no tickets: say you cannot list its tickets from here, and point the user to the job\'s Service Tickets tab (or the Service Tickets section on the lead), or to the Service Tickets page in the sidebar (/service-tickets), which lists every ticket the user can see, with status pills, filters and views such as My approvals, Overdue and Unassigned.',
    'SERVICE TICKETS (work orders on a job or a lead): to draft one, READ the job or lead first, then describe the ticket to `scribe_write` — the parent (the job by its number or id, or the lead by id), the title, the scope, the scheduled and due dates as real calendar days, the person doing the work as the work order\'s own assignee (`assignee_user_id N` — see Assignees), and each child task — a building on its punch list — with its title, due date and notes ONLY — and say REPLY ONLY for a child task that is a call, an email or a confirmation rather than field work (it is checked off without a completion photo). Never give a child task an assignee, not even the same one on every task: everyone on the work order shares every building, and a child task carrying an assignee makes the Scribe refuse the WHOLE ticket. A ticket\'s status, its approved scope and a move to a different job or lead are changed in the ticket by a person, not through the Scribe — say so rather than trying. Never tell the user a ticket exists until that payload has been approved.',
  ...
  scribe: [
    ...
    '  • estimate: `{op, scope?, field_updates?, sections?, groups?, line_adds?, line_edits?, line_deletes?, assembly_adds?}`. line_adds: `{description, qty, unit, unitCost, markup?, subgroup_id?, section?}` (camelCase `unitCost`/`markup`; snake_case aliases also accepted).\n' +
    ...
    '     - change_orders fields: `{title, lines:[{description, qty, unitCost, markup?, unitSell?, costPending?}], scope?, terms?, co_number? (create only), defaultMarkup?, targetMargin?, feeFlat?, feePct?, taxPct?, roundTo?, notes?, building_id?}`. Income = lines through markup → target-margin → fees → tax (exactly the CO editor\'s math); cost = the raw line subtotal.\n' +
    '     - **`fields.lines` REPLACES THE WHOLE ARRAY. To change ONE line on an EXISTING change order, use the per-line ops instead** — a change_orders op may carry `line_edits: [{line_id, unitCost?, unitSell?, qty?, unit?, description?, markup?, costPending?}]`, `line_adds: [{description, qty, unitCost, ...}]` and `line_deletes: [{line_id}]` beside (or instead of) `fields`. They touch the lines they name and leave every other line exactly as stored. Sending `fields.lines` with only the line you were told about DELETES THE REST and still reports success — that is silent revenue deletion, and it is why these ops exist.\n' +
    '     - Addressing: `line_id` is what `read_change_orders` prints as `[line_id=…]`, and it is the right address. `line_number` (the `#3` in the same read) also works for a change order old enough to have no line ids. `co_id` accepts the co_ row id or the CO number ("CO-3"). An unresolvable line or change order is REFUSED with the record\'s actual contents listed — it is never counted as an applied edit.\n' +
    '     - Editable line fields are exactly: description, qty, unit, unitCost, unitSell, markup, markupMode, costPending. Anything else is refused by name. Section headers and assembly provenance are not writable this way; a costed recipe goes on through `assembly_adds`.',
    '     - **`unitCost` is WHAT THE WORK COSTS — never a price quoted to the owner.** If you put a quoted price there, the change order books the whole quote as job cost and reports $0 profit. When the number you have is a PRICE (a "flat rate" off a proposal), put it in `unitSell` — the promised per-unit price, which overrides the markup cascade — and put the real cost in `unitCost`. If the cost is genuinely unknown, set `unitCost` to the same number and `costPending: true` rather than guessing: that is the conservative reading and it flags the line for a human instead of hiding it.',
    '     - **A change_orders op may carry `assembly_adds: [{assembly_id (int), params:{Q, ...}, mode?}]` beside (or instead of) `fields`** — puts a costed ASSEMBLY on the change order as ONE LINE per cost bucket, INTACT, exactly as `estimate.ops.assembly_adds` does. USE IT whenever the instruction names an assembly/recipe: the recipe supplies the COST and the markup cascade supplies the price, so you never hand-write a unit cost. Leave `mode` off (defaults to `rollup`) — exploding a line into components is a HUMAN action taken later in the editor. A refusal (unpriced items, formula error, Q ≤ 0) is a HARD STOP: do NOT retry and do NOT substitute hand-written lines with your own unit costs. A change order is one flat group, so there is no alternate to name.',
    ...
    '  • system: `{skill_pack_ops?, field_tool_ops?, link_ops?}`. link_ops includes `{op:\'attach_files\', attachment_ids:[...], target_entity_type, target_entity_id}` — which MOVES files to a different parent entity and writes NO metadata. It is NOT the caption op: a `caption`, `captions` or `tags` key handed to it is IGNORED while the rows are re-pointed to the parent you named, so aiming it at a guessed parent moves photos and reports success. Never reach for it to write a description.',
    '  • attachment: `{photo_updates: [{attachment_id, caption?, tags?}, ...]}` — the DESCRIPTION and TAGS on photos that are already uploaded. This is the ONLY op that writes a photo\'s description. The target carries NO entity_id: `{entity_type:\'attachment\', ops:{photo_updates:[…]}}` — and for the same reason it is never a `bulk`, a `condition` or a side of an `op:\'move\'`; each of those is a second address for a write that already has one, and each is refused by name.\n' +
    '     - `caption` IS the photo\'s description — the same field the photo grid and read_project_photos show as "caption". Max 2000 chars. Pass an empty string to clear one.\n' +
    '     - `tags` REPLACES the photo\'s tag array — send the full list you want it to end up with, not just the new ones. Case is preserved. ABOVE THE LIMITS NOTHING IS QUIETLY DROPPED, THE WHOLE PAYLOAD IS REFUSED: more than 20 tags, a tag over 32 chars, a blank tag, a non-string entry, or the same tag twice (they dedup case-insensitively) each refuse by name and save nothing. Send tags you have already trimmed to fit rather than relying on the write to shorten them — it will not.\n' +
    '     - Addressing: `attachment_id` and nothing else — never an index, a position or a filename. It is the id `read_project_photos` prints in square brackets at the head of each line, for a photo on ANY parent that read covers (lead, job, project, task) — this write has never cared which parent a photo hangs on, only that the id resolves and you hold the edit capability for that parent. An id that does not resolve, or belongs to another organization, is REFUSED — never skipped, never counted as an applied edit — and the refusal prints the parent\'s file list when another id in the same batch has already resolved.\n' +
    '     - ONE target holds the WHOLE set — 42 photos are 42 entries in one `photo_updates` array, one payload, one approval card. Do NOT emit one target per photo. Max 60 entries per target; narrow the set with read_project_photos\' `missing_caption_only` / `tag` / date filters and emit the rest as a second payload.\n' +
    '     - NOT the same thing as `report.ops.section_updates[].captions`, which is a per-figure caption inside a REPORT\'s JSONB. If the instruction says "describe these photos", it is `attachment.photo_updates`. If it says "caption the figures in the report", it is the report op.',
    ...
    '  • task: `{op:\'create\', fields:{title, due_date? (DATE), notes?, kind?, status?, priority?, assignee_user_id?, entity_type?, entity_id?}}` — ORG work: org-visible and assignable. kind: todo | punch | follow_up. status: open | in_progress | blocked | done. priority: low | normal | high | urgent. assignee_user_id must be a real in-org user id, written as a NUMBER exactly as the instruction gives it (e.g. `assignee_user_id 2`). NEVER put a name or an email there — you cannot look people up, and the dispatcher refuses it. If the instruction names a person but gives no id, do not guess and do not quietly drop them: return one line saying which person\'s user id is missing. Leave it out only when the instruction names nobody; it then defaults to the acting user.',
    ...
    '  • service_ticket: `{op:\'create\'|\'update\', fields:{...}, task_adds?:[...]}` — a WORK ORDER raised on a job or a lead: the field work to be done, where, when and by whom.\n' +
    '     - create: `fields.title` is required, and so is a parent — `fields.job_id` or `fields.lead_id`, at least one. job_id is a real job — its id, or its job NUMBER exactly as the instruction gives it (e.g. "RV2006") — and never a `$new_` ref, because no payload creates a job; lead_id is a real lead id or a `$new_<name>` ref to a lead created earlier in the same payload. A create carries NO real entity_id (omit it, or use a `$new_<name>` ref) — the id is minted when the ticket is written. A new ticket starts as a draft.\n' +
    '     - update: `op:\'update\'` with entity_id = the ticket\'s real id; send only the fields that change.\n' +
    '     - fields: title, scope_proposed, internal_notes, priority (low | normal | high | urgent), requested_by, site_contact_name, site_contact_phone, street_address, city, state, zip, lat, lng, access_notes, scheduled_for (DATE), due_date (DATE), assignee_user_id (a real in-org user id) written as a NUMBER — never a name or an email, materials (optional takeoff: [{description, qty, unit}] — quantities only, NEVER a price or cost; draft it only from a takeoff in the job files or what the PM tells you). A DATE is a calendar day written YYYY-MM-DD — resolve a real day, never a relative phrase.\n' +
    '     - task_adds: `[{title, due_date? (DATE), priority?, notes?, kind?}]` — child TASKS created under the ticket, beside `fields`, on a create or an update. They are ORG work, never personal to-dos; they belong to the ticket\'s job or lead, so do not pass entity_type/entity_id for them.\n' +
    '     - A task_adds entry is a BUILDING on the work order\'s punch list, and A BUILDING IS NEVER ASSIGNED TO ONE PERSON. There is no per-building assignee to write: the one Assigned to that means anything is the work order\'s OWN `fields.assignee_user_id`, and everyone on that record is equally responsible for EVERY building on its punch list. So when the instruction names who is doing the work, put that person on `fields.assignee_user_id` ONCE — never on the task_adds entries, and never split the crew building by building. A child task whose work is a call, an email or a confirmation is REPLY ONLY: give it `kind:\'follow_up\'` and it can be checked off without a completion photo; leave kind out for field work, which needs photo proof. If the instruction gives the child tasks an assignee and it is the SAME person on all of them, that person IS the work order\'s assignee: put them on `fields.assignee_user_id` once and give the task_adds none. If it gives DIFFERENT people to different child tasks, do not draft — return one line saying a work order has one Assigned to, shared by every building, and ask who should own it.\n' +
    '     - REFUSED, and none of these is fixed by re-emitting in another shape. Do NOT retry, and do NOT drop or swap a field to get past one — return the refusal so the user hears it: `assignee_user_id` on a `task_adds` entry (a building on a work order is never assigned to one person — the person goes on the work order\'s own `fields.assignee_user_id`, which everyone on the record shares for every building); `status` (a ticket\'s status is changed in the ticket, where its lifecycle is enforced — the user changes it there); `scope_approved` (the approved scope is what the client or PM signed off on, and a person records it in the ticket — it is never authored by you); `job_id` or `lead_id` on an update (a ticket is never re-parented — the user raises a new ticket on the other job or lead); any edit to a closed or cancelled ticket (the user reopens it in the ticket first); a parent job or lead that does not resolve in this organization; a `condition` on a service_ticket target (if_exists / if_missing / upsert — a create makes a new ticket and an update names an existing one); a service_ticket as EITHER side (source or dest) of an `op:\'move\'` (a ticket is updated in place and never re-parented).\n' +
    '     - The payload `title` and `summary` become push-notification titles and chat headers, so they name the ticket by its TITLE ONLY — never its scope, its notes, the site contact\'s name or phone, or the address.',
    '',
    ...
    '  • service_ticket.fields: title, job_id (create only), lead_id (create only), scope_proposed, internal_notes, priority, requested_by, site_contact_name, site_contact_phone, street_address, city, state, zip, lat, lng, access_notes, scheduled_for, due_date, assignee_user_id, materials ([{description, qty, unit}], no price keys). priority enum: low | normal | high | urgent. NOT writable from a payload, each refused by name: status, scope_approved, checklist, guest_log, ticket_number, and every *_at timestamp.',
  ...
  assistant: [
    ...
    '- To CHANGE anything, you do NOT edit data yourself — you hand a fully-specified change to the Scribe via `scribe_write` (your writer; for a small change — plain fields on ONE record, or ONE new to-do/task/reminder/calendar event — use `quick_write`, drafted in this turn). APPROVAL: every draft reaches the user as ONE line with an Approve button. Do not ask "shall I?" first — a yes before that line exists approves nothing. When the user says yes AFTER the line appears, call `approve_pending_write` (no input) and reply with only the receipt line it returns; if it refuses, relay that in one line. Deletes, status changes, money and % complete always need a tap. When EDITING an existing record, resolve its entity_type + entity_id from your reads FIRST, then describe the change in plain words including every field and value to set.',
    '- WRITES RUN IN THE BACKGROUND: the Scribe does the work AFTER `scribe_write` returns, and the outcome (a one-line card with Approve) arrives on its own. So do NOT report a change as already done in the same breath — say you have "sent it to the Scribe" or "queued the change," never "Done, I updated it," and do not state a new value as fact until you have re-read it. NEVER invent a workaround: `scribe_write` is your only writer (and `approve_pending_write` only answers a yes to its card) — do not mention `emit_payload_file` or any other write tool, never say a tool was "blocked," and if `scribe_write` is not in your live tool list this turn, tell the user plainly you cannot make that change right now instead of implying you did.',
    ...
    '- SERVICE TICKETS are work orders on a job or a lead. Read one with read_entity entity_type `service_ticket`, by its id only. You CANNOT list them: no job or lead read lists its tickets and none is searchable, so NEVER say a job or lead has no tickets — say you cannot list them from here and point the user to the job\'s Service Tickets tab (or the Service Tickets section on the lead), or to the Service Tickets page in the sidebar (/service-tickets), which lists every ticket the user can see, with status pills, filters and views such as My approvals, Overdue and Unassigned. To draft one, read the job or lead first, then describe it to scribe_write — the parent, title, scope, dates and any child tasks. It does not exist until that change is approved, so never say it does before then.',
...
function builtinToolsetFor(agentKey) {
  ...
  // quick in-chat lookups, NOT the full 8-tool sandbox (~30k of schema) dropped in
  // dae1a53. Heavy sandbox work (bash/Python → Excel/PDF) stays on 86 via the
  // background-tasks path. (The 2026-06-30 "no response" outage was NOT this config —
  // it was a TDZ bug in the day-orient injection, fixed in 1305e32. The skills
  // read-gate doesn't bite here: the assistant runs with no toolset today, which
  // itself would fail session.create if any skill were attached — so it provably has
  // none. If a skill pack is ever mirrored onto the assistant, enable read/glob/grep
  // here alongside web or session.create will start failing.)
...
function customToolsFor(agentKey, opts) {
  ...
  // lands as a one-line card (a chat yes may apply a low-risk one via
  // approve_pending_write). It is NOT
  ...
  if (agentKey === 'assistant') {
    ...
    const ASSISTANT_TOOL_NAMES = new Set([
      ...
      'scribe_write',
      // …and the yes to a one-line card already on screen (server-bound)
      'quick_write',          // a small change drafted in this turn (no Scribe)
  ...
  if (agentKey === 'job') {
    ...
    const ROUTER_TOOL_NAMES = new Set([
      ...
      'read_projects', 'read_project_photos',   // project photo feed: ids, captions, tags, geo
      'read_purchase_orders',
      'read_change_orders',    // change orders + their LINE ITEMS, with the ids a write needs
      // ── Navigation (1) ──
      'navigate',
      ...
      'scribe_write',
      'quick_write',           // a small change drafted in this turn (no Scribe)
      'approve_pending_write', // a chat yes to a one-line card on screen — server-bound
...
// Exported for the same reason ai-routes exports its internals: a claim about
// what 86 is TOLD can only be held by reading the thing it is actually sent.
// test/agent-instruction-honesty.test.js cross-checks every "# Block" named in
// here against the blocks the context builders actually emit on the live path —
