...
const AGENT_SYSTEM_BASELINE = {
  ...
  job: [
    ...
    'SERVICE TICKETS (work orders on a job or a lead): to draft one, READ the job or lead first, then describe the ticket to `scribe_write` — the parent (the job by its number or id, or the lead by id), the title, the scope, the scheduled and due dates as real calendar days, the assignee if one was named, and each child task (title, due date, assignee). A ticket\'s status, its approved scope and a move to a different job or lead are changed in the ticket by a person, not through the Scribe — say so rather than trying. Never tell the user a ticket exists until that payload has been approved.',
