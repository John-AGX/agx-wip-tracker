...
const releases = [
  {
    ...
    changes: [
      ...
      { type: 'improved', text: 'Two limits on inbound attachments, both worth knowing before you rely on them. A single file larger than 18 MB is not kept, and nothing on screen tells you — someone who sends you one 40 MB drawing gets no error, and the message lands in your hub looking like it never had an attachment. Several files together are worse: past about 18.5 MB of files in one message the app refuses the whole delivery, the gateway hands it back and keeps retrying, and the mailbox that forwards into your dropbox eventually gets a bounce notice — so there you lose the mail, not just the files. If you are expecting something big, that is what to watch for. Second, email rules and attachments. A rule whose only condition is "Has attachment" now fires correctly on arriving mail, and that is the only kind of attachment rule you can build, because the Rules screen takes one condition per rule — so pairing "Has attachment" with Auto-category, Urgency or Needs a reply is not something you can set up today. One thing that is deliberate rather than a limit: if storing a file fails, the message still arrives. You lose the file, never the mail.' },
