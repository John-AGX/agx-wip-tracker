...
const releases = [
  ...
  },
  {
    ...
    changes: [
      ...
      { type: 'new', text: 'Market mapping. Buildertrend sends a market as an option number, never its name, so the Jobs and Clients tabs now list each option with the evidence to decide from: how the records carrying it that are already linked are filed in Project 86, and where the work is — a job by its site, a client by its property address, with where the client is billed shown separately, because a management company billed from one state often runs properties in another. When the linked ones agree, the market is marked as suggested; you choose. Once an option is mapped, a job or client with no market gets it, one filed under a different market waits for a tick, and a new one arrives already filed. A record with no market chosen in Buildertrend is left alone; it is not an option to map.' },
  ...
  },
  {
    ...
    summary: 'A Buildertrend estimate worksheet can be brought into Project 86 as a real estimate, with its line items, its sections and its markup, attached to the job it belongs to — and it refuses rather than guessing wherever Project 86 cannot reproduce Buildertrend’s own price for a line.',
    changes: [
      ...
      { type: 'improved', text: 'Markup comes across as the percentage Buildertrend itself records, and every line is checked before it is taken. Buildertrend writes the same markup four ways on every line — a percentage, a per-unit amount, a flat amount and a margin — and which one you typed is kept beside them as a code, so the percentage is what is read. It is not trusted, though: each line is priced from its own quantity, unit cost and that percentage through the same pricing the estimate editor and the proposal use, and compared with what Buildertrend says the owner pays for that line. A line that agrees is imported. A line that does not agree is refused, naming both figures — and a refused line refuses the whole worksheet, because an estimate missing lines is a wrong document rather than a partial one. A line marked up below its cost is imported exactly as Buildertrend priced it — at -100% Buildertrend charges the owner nothing for work that still costs money — and the row says which lines those are.' },
      { type: 'improved', text: 'The money is checked, not assumed. Every worksheet offered is priced through the same pricing the estimate editor and the proposal use, and compared with Buildertrend’s own owner price. Any difference larger than Buildertrend’s own percentages can account for is shown as a held-back item naming both figures, never quietly absorbed. Contract price and line costs are always ticked by hand, never applied by “Link confident matches”.' },
  ...
  },
  {
    version: '1.31',
    date: '2026-09-18',
    name: 'Warranty, pending change orders, and closing a paid PO',
    summary: 'A job can be in Warranty, a change order can be waiting for the owner to sign it, and a purchase order Buildertrend has approved, finished and paid can be closed from the Buildertrend preview — one row at a time, never automatically.',
    changes: [
      { type: 'new', text: 'Warranty is a job status. A job in warranty is still an active job: its contract, WIP, backlog, revenue earned, profit and margin are exactly what they were, it still counts in Active Jobs and in market and Insights figures, and it can still raise purchase orders, change orders and RFIs. You can set it on the job’s Information card, in the New Job box, from the jobs list bulk menu and in the Jobs filter, and it has its own violet badge everywhere a job status is shown. On the Schedule board it has its own pill, on by default — if you had already chosen which statuses to show, Warranty is added to your choice without touching the rest, unless you had every pill off, which already shows everything. Buildertrend jobs marked Warranty now propose Warranty in the Buildertrend preview instead of a note nobody could act on, and a warranty job created from Buildertrend starts as Warranty with its Notes left empty (it used to have a line of machine text written into the job’s own notes). The jobs list and the New Job box also gained the Backlog option they had been missing.' },
      { type: 'new', text: 'A change order can be Pending approval. It means written up and sent to the owner, with nobody having signed it. A pending change order is worth exactly $0 — no contract total, WIP, backlog, pay application, building card or job figure moves, precisely as a draft does not — and it stays fully editable and unlocked. Move a change order to it from Draft with the status pill in the change-order editor or the bulk Set status menu, and from there to Approved or back to Draft. Applied is only reachable from Approved: the field cannot have consumed something nobody signed. It has its own amber pill, the Change Orders list has a Pending approval filter, and the Summary page’s change-order card counts it. Buildertrend change orders marked Pending now land as Pending approval instead of quietly staying drafts.' },
      { type: 'new', text: 'Close a purchase order from the Buildertrend preview. When Buildertrend has a purchase order approved, its work Complete and its payment status Paid — exactly Paid, not Partially Paid — the Purchase orders tab offers to close the matching P86 purchase order. It is offered per row, never ticked for you, never applied by “Link confident matches”, and the confirm says plainly that closing is permanent: a closed purchase order cannot be edited, unlocked, revised by addendum or deleted by anyone, and it drops off the Purchase orders hub’s open list. Its cost does not change and its sub keeps portal access to the job’s files. The offer is refused while the purchase order has an addendum awaiting a signature, and Buildertrend’s paid status is now shown on every purchase-order row so you can see what you are agreeing to.' },
      { type: 'fixed', text: 'Close PO works on the purchase-order page again. Every purchase order past draft is locked, and the workflow button was hidden on a locked purchase order — so a purchase order at Work complete had no way forward at all. The Close PO button is back, and wherever you press it — the purchase-order page, the Purchase orders bulk menu or the Buildertrend preview — it now warns, in red, that closing is permanent: no edit, no unlock, no addendum, no delete, by anyone.' },
      { type: 'fixed', text: 'A closed purchase order can no longer gain an addendum. Recording or approving an addendum was the one purchase-order action that did not check whether the purchase order was closed, so approving one that was already waiting raised a closed purchase order’s committed total. It is now refused, like every other change to a closed purchase order.' },
      { type: 'fixed', text: 'Archiving a job and putting it back keeps its status. Unarchive used to set every job to Completed and Restore on the Archived tab set every job to In Progress, whatever the job had been. Every way of archiving a job — the job page, the jobs-list bulk menu, the Job Information card and the Buildertrend sync — now records the status it was archived from, and both ways back put it there, so a Warranty, Backlog, New, On Hold or Completed job comes back as itself.' },
      { type: 'fixed', text: 'A pending change order no longer shows the word “Applied” on the job screen, is not described to 86 as a draft, reads “Pending approval” rather than a bare “Pending” on the Change Orders list, and shows its own chip on the work order it was started from.' },
      { type: 'fixed', text: 'A purchase order that is part-way through a revision is not offered the close. If it is unlocked to revise, or its line items no longer add up to what it has committed, there is a price change nobody has recorded — and because a closed purchase order counts its committed total, that difference would not be visible on the preview row. The offer is withheld with the reason shown: record the change as an addendum first. A close is also applied to one purchase order at a time, never to a list of them under a single press.' },
      { type: 'fixed', text: 'A change order Buildertrend has declined is flagged when Project 86 has it pending approval, not just when it is a draft. It used to be a passive note, so it never reached the preview’s Flagged filter.' },
      { type: 'fixed', text: 'A Buildertrend job marked Warranty proposes Warranty against an active or completed Project 86 job, and proposes nothing against an archived one — archiving is a decision the sync does not undo on its own.' },
      { type: 'improved', text: 'A jobs view you saved before Warranty existed, or a change-order view saved before Pending approval, keeps the statuses you picked and therefore leaves the new one out. Tick it on the view and save again to include it.' },
    ],
  },
  {
    version: '1.30',
