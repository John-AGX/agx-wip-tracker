...
const releases = [
  ...
  },
  {
    ...
    changes: [
      { type: 'new', text: 'Take photo on the punch list. On a phone or tablet, each building can take its before and completion photos with the camera, on the crew link and on the work order in the app. Take photo opens the camera straight away and saves the shot to that building, one photo a tap; Upload still opens the photo library and takes many at once. On Android the library has no camera in it, so until now a crew could only add photos they had already taken. On the crew link, completion photos end in a Take photo tile and an Upload photo tile (until now Add photo), and Take before photo sits next to Upload before photo (until now Add before photo). In the app on a phone it is usually the same, and the completion buttons under the photos are gone because the tiles do their job. On a wider screen, such as most tablets or a phone turned sideways, the app shows Take completion photo, Upload completion photo, Take before photo and Upload before photo (until now + Completion photo and + Before photo). The field report at the bottom of the crew link has Take photo too, next to its Upload photo (until now Add photo), which adds one photo at a time. On a computer, including a Windows tablet, there is no Take photo, because the browser there opens the file picker instead of the camera; Upload works as before.' },
    ],
  },
  {
    version: '1.27',
    date: '2026-09-13',
    name: 'Takeoffs, Phones and Mail',
    summary: 'Fill a work order’s material list from a takeoff already on the job, let the crew link open that takeoff without showing prices, a proper phone layout for the job page and work orders, and company mail sent in your company’s name with replies that reach a person.',
    changes: [
      { type: 'new', text: 'Fill a work order’s materials from a takeoff. In a ticket’s Materials editor, Fill from a job file lists the spreadsheets, CSVs, PDFs and photos already attached to the ticket’s job, its lead and its estimate (lead and estimate files only for people who can see those). Pick one and its material lines land in the editor as description, quantity and unit — nothing is saved until you press Save materials, and if the list already has lines you choose Replace or Add to it. Spreadsheets and CSVs are read directly, by their column headings, including the AGX Lead Report, Buildertrend estimate exports and Home Depot purchase lists; labor lines, subtotal rows, removed lines (quantity 0) and returns are skipped and the note under the editor says how many. PDFs and photos, and spreadsheets whose columns can’t be matched, are read by AI and marked “Read by AI — check every quantity before saving.” Prices are never copied: price, cost and total columns are not read, and money written into a description is removed. Up to 100 lines are read; past that the note says the rest were left out. Old .xls files can’t be read — save them as .xlsx first. One file is read per person at a time.' },
      { type: 'new', text: 'The crew link can carry the takeoff — without prices. Under Materials, Show a takeoff file on the crew link picks a takeoff from the same job, lead and estimate files, and the crew link then shows a Takeoff card. For a spreadsheet (.xlsx or .csv), a price-free copy is made when you pick it, with the same reader Fill from a job file uses: a sheet holding only material, quantity and unit. Crew links that hide financials — the default — get that copy (Materials list, prices removed); the original spreadsheet only goes on links you send with financial details. If no copy can be made, including from an old .xls, the file only shows on links sent with financial details, and the office row says why. PDFs and photos can’t be copied that way, so you are asked to confirm the crew may see the whole file, and it shows on every link. Files open through the link itself, so they stop opening when the link is turned off or expires, or the file is removed from the job. Files over 25 MB can’t go on the crew link. After a Fill from a job file, Also show this file on the crew link does the same in one step.' },
      { type: 'improved', text: 'The job page on a phone. The money tiles are one row you swipe, with their full labels, instead of being squeezed to a letter each; the sections (Overview through Service Tickets) are one row you swipe, with the open section centred; Edit, Archive and Delete sit behind a ⋮ More button; and the bottom bar no longer covers the end of the page. On a computer nothing changes; in a narrow window with a mouse the rows wrap instead of swiping.' },
      { type: 'improved', text: 'Work orders on a phone look like the crew link. An open ticket is a stack of full-width cards: the job site with the job number, address, a large Navigate button and the gate code; the status steps in one row with the current step centred; the scope; the details in two columns; materials with each quantity in its own box; and one card per building with a Done or Needs photo tag, its Side A and Side D items, photo tiles with an Add photo tile, notes and the complete box. The crew link got the same treatment: the status steps in one row, quantities in boxes, and completion photos as tiles with Add photo. Buttons with white text on blue were too faint in dark mode and now use dark text.' },
      { type: 'new', text: 'Mail from your company is sent in your company’s name. Notifications and links sent on a company’s behalf now come from “<your company> via Project 86”, still from the Project 86 address — task and job assignments, messages, schedule notices, report, task and crew links, sub invites and assignments, proposals, lead sold or lost, work orders ready for approval, reminders, weekly digests, certificate expiry notices, campaigns and background task notices. Password resets, new-user invites and company invites stay plain Project 86. A company name is refused when it is saved if it spells or imitates Project 86, hides characters, mixes lookalike letters from different alphabets inside a word, or puts a symbol inside a word (a space fixes “ABC建設” or “Acme®”); such a name is never used as a sender, and a company already named that way can still save its other settings.' },
      { type: 'new', text: 'Replies reach a person. When someone replies to one of those emails, the reply goes to the person behind it: whoever assigned the task or job, sent the message, shared the link, sent the proposal, changed the lead, scheduled the crew, or finished the work order; certificate expiry notices reply to the company’s first admin. The address always comes from that person’s current account in your company. Until now no email from the app carried a reply address at all, because of a bug in how it was passed to the mail service, so replies went back to the sending address instead of to a person.' },
      { type: 'fixed', text: 'Emails that could reach another company. A direct-message email could be sent to a user in another company, schedule crew could include people from another company, a task shared with a sub could fill in another company’s sub, a lead sold or lost notice could reach another company’s salesperson, and editing a schedule entry could move it onto another company’s job. Each is now limited to your own company. Proposal emails are limited to 20 per person per hour, and their default subject names your company instead of AGX.' },
    ],
  },
  {
    version: '1.26',
    ...
    summary: 'When a work order is finished, the people who approve it are told — by email and on their phone — instead of having to notice it in a list.',
    changes: [
      { type: 'new', text: 'A finished service ticket now tells the office. The moment a ticket reaches Work complete — the status the Awaiting approval filter lists — the people who approve it hear about it by email and on their phone. It fires whichever way the ticket gets there: the crew finishes the last building, the crew presses Mark work complete, or someone in the office moves it to Work complete. Who hears: the job’s project manager and whoever raised the ticket, plus — when the crew finished it through a link — whoever sent that link. Each person is told once, never the person who made the move themselves, and only if they can still approve it — edit its job or lead: someone taken off the job since, left with view-only access, or whose account is switched off hears nothing. The email says who finished it and how, names the job (or the lead) and its address, gives the punch-list tally (“21 of 21 subtasks done · 34 completion photos”) and has a Review and approve button that opens the job’s Service Tickets tab with that ticket already expanded; a ticket on a lead opens the lead. The phone notification names the job, the ticket and who finished it, and opens the same place. Nothing about money is in either, and a name the crew typed on the link is shown as plain words only — anything that looks like a web address is left out.' },
      { type: 'new', text: 'One notice per arrival, not per click. If the crew finishes the last building, undoes it and finishes it again within 15 minutes, the approvers are told once. If the office sends the ticket back — moving it to In progress, Scheduled or Open, or unticking a building — the next time it reaches Work complete is announced again straight away. Each notice lands on the ticket’s Progress list as “Project 86 told … it is ready for approval”, with the names of the people it actually reached, so whether the office heard about it is on the ticket itself. If nobody could be reached — everyone has it turned off, or email failed and nobody has phone notifications on — no such line appears, and the next arrival is still announced.' },
      { type: 'improved', text: 'Turn it off in My Account → Notifications under Work orders to approve, separately for email and push. The phone notification only reaches a device where notifications are enabled — on iPhone, add the app to the home screen first.' },
    ],
  },
  {
    version: '1.25',
    ...
    changes: [
      ...
      { type: 'new', text: 'The ticket follows its buildings. When the last building is marked complete on an Open, Scheduled or In progress ticket, it moves to Work complete by itself — so it shows under the Awaiting approval filter — and says so in the Progress list (“moved it to Work complete — every subtask done”); reopen a building and it moves back to In progress. It does not go further than that on its own — approving is still yours, with Move to… → Approved. Once a ticket is approved, the crew can no longer change a building; the office still can.' },
      ...
      { type: 'improved', text: 'Mark work complete on the crew link now says what it does: it tells the office the whole job is done and ready for approval, moving the ticket to Work complete, where it shows under the Awaiting approval filter. Finishing every building does the same thing by itself, so the button is for work that is not broken into buildings. At this release neither sent anyone a notification — the ticket showed under Awaiting approval and in its Progress list. 1.26 adds the notice.' },
  ...
  },
  {
    ...
    summary: 'Building cards have been hiding their change orders since August, including credits that were being billed on the G703; the allocation table takes cents; 86 stops being told things about itself that were not true; and a receipt now says which counter it came from.',
    changes: [
      { type: 'fixed', text: 'Estimate lines that took your typing and threw it away. Version 1.20 said the writer and the screen "now agree on what an address is." That was true of the case it fixed — an address stored as a plain number where the screen can only hold text — and not true of eight more shapes. An address carrying an apostrophe, a backslash or an invisible line break left the row inert: every field took your typing and discarded it, delete removed nothing, and the save pill read "No changes" the whole time. Thirty address shapes now go through both the estimate editor and the change-order editor and the two agree on all thirty; the change-order editor had been losing three of them. This fix rewrites nothing stored — the separate 1.20 repair, which fixes an unusable address as the estimate loads, still runs.' },
      { type: 'fixed', text: 'Building cards have been hiding their change orders since 2 August. Each card rendered "CHANGE ORDERS (0) — No change orders allocated to this building" on every job, unless that browser still carried legacy node-graph wiring. The card looked change orders up under a name the records do not use. Fixed along with the status filter that goes with it, so draft change orders stay off a card that shows money — and with a second fault the first one had been masking: the card dropped any change order worth a negative amount, so a deductive change order showed on the G703 as a credit against that building while the card said none was allocated. It was the only one of the three per-building change-order readers that dropped a negative; the Site Plan and the pay application both carry them deliberately.' },
      { type: 'fixed', text: 'The contract allocation table would not take cents, and the split invented the ones it could not hold. The dollar cells stepped in $100 jumps and flagged cents as invalid — on a phone the entry was refused outright — and behind them the percentage split rounded to whole dollars, so a scope total of $10,000.50 allocated $10,001.00, fifty cents that were never yours, with the row total and the column footers disagreeing. Cells take cents now, a three-way split absorbs the odd sub-cent so the row still adds up exactly, and money that genuinely is not allocated is shown as a figure rather than rounded into a green tick. One thing does move, and it is the point: a percentage scope already split into whole dollars re-splits to the cent the next time it is committed — on a $10,000 three-way the largest cell moves 66 cents. The row total does not change.' },
      { type: 'fixed', text: '86 could not open a change order, and 1.20 said it could. That release announced you could ask what is on CO-3 and get the lines back; the tool had shipped broken the day before and stayed broken. From 23 August until this release it failed on every call — it asked for a job number from a column that does not exist. It had no other way in: the summary 86 receives for a job does not list change orders either. So for twelve days, asking 86 about a change order got an error rather than an answer.' },
      { type: 'fixed', text: 'Asking 86 to search your knowledge base has never worked. Since the search shipped in May, both the personal and the organisation-wide search failed on every call, for the same reason: they asked attachments for a date column they do not have. Nearly four months in which 86 could not search your files for you. Reading a file it had already been handed was unaffected, as was browsing them yourself — it was keyword search that was dead.' },
      { type: 'fixed', text: '86 was being told things about itself that were not true. Its instructions said the summary it receives for every job lists that job’s change orders, quoting the format — it never has, on any live path, since 17 May, so 86 was describing money records it could not see. A sweep of the rest found more of the same across the app: instructions naming blocks that are not built, including one deleted in August, and two telling it to stop and read a block that does not exist. All corrected, and a test in the suite now goes red if an instruction points at a named block the app never produces. That test catches the common form and not every form — a reference written without the leading hash still gets past it.' },
      { type: 'fixed', text: 'Voiding a receipt did nothing on your phone. The confirmation used a browser dialog that does not exist in the installed app, so the button silently did nothing at all — no question, no void, no error. Same for naming a saved view. Both now use the app’s own dialogs.' },
      { type: 'new', text: 'A receipt now tells you where you bought it. Capturing one reads the merchant, the store number, the street address and the phone off the receipt itself, and a new Merchants view shows every supplier your receipts and QuickBooks costs already name — how often, how much, and which branches. Nothing is saved as a vendor record and nothing is created: the view is read off the receipts and QuickBooks lines you already have. A phone read once is marked unconfirmed rather than shown as a number to call, a fax line or a national line is labelled as what it is instead of as the branch’s own, and a premium-rate number is never made tappable.' },
      { type: 'fixed', text: 'Deleting a sub could orphan a purchase order, a bill or a receipt. The check before deleting only looked at job assignments — the one thing the database already refused on its own — so a sub carrying live money pointers deleted cleanly and left them pointing at nothing. It now refuses and names what is holding it, with counts.' },
      { type: 'new', text: 'Your company’s Project 86 email addresses are yours to set. In Admin → Users an org admin can give anyone in their company a proper address — john.agx@project86.net rather than the auto-generated one — and each company has its own namespace, so two companies can both have a john. Every address a person has ever held goes on resolving to them for as long as their account is active, including the auto-generated one and anything they had before a rename, and a retired address stays claimed rather than being handed to someone else. One rough edge to know about: setting an address back to one that same person used to hold is refused today as though somebody else had taken it.' },
      { type: 'improved', text: 'The sidebar reads as a hierarchy again. A main entry and its children sat half a pixel apart, so depth was carried by indent and icon size alone; the whole scale is now a step smaller, with the gap between a main entry and its children doubled.' },
      { type: 'improved', text: 'A magnifier on each building card frames that building on the Site Plan. It appears in the inspector beside the map, where zooming is a zoom rather than a navigation jump — and when a building has never been traced it says so and points you at Trace Building with that building already selected, instead of appearing to work and doing nothing.' },
    ],
  },
  {
    version: '1.22',
