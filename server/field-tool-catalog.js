...
'    var c=document.getElementById("stringer"), o=ctxFit(c,260), x=o.x, W=o.W, H=o.H;',
'    var padL=40,padR=52,padT=30,padB=44;',
'    var sc=Math.min((W-padL-padR)/Math.max(M.run,1),(H-padT-padB)/Math.max(M.rise+M.bwidth*0.7,1));',
'    var ox=padL, oy=H-padB; function PX(r){return ox+r*sc;} function PY(r){return oy-r*sc;}',
'    var L=Math.sqrt(M.run*M.run+M.rise*M.rise)||1, nx=M.rise/L, ny=M.run/L;',
'    var cut=[[0,0]],cx=0,cy=0; for(var i=0;i<M.risers;i++){ cy+=M.riser; cut.push([cx,cy]); if(i<M.treads){ cx+=M.tread; cut.push([cx,cy]); } }',
'    var bA=[0-nx*M.bwidth,0-ny*M.bwidth], bB=[M.run-nx*M.bwidth,M.rise-ny*M.bwidth];',
'    dashed(x,PX(bA[0])-12,PY(bA[1]),PX(M.run)+8,PY(bA[1]));',
'    dashed(x,PX(M.run-M.tread),PY(M.rise),PX(M.run)+40,PY(M.rise));',
'    x.beginPath();x.moveTo(PX(cut[0][0]),PY(cut[0][1]));',
'    for(var j=1;j<cut.length;j++) x.lineTo(PX(cut[j][0]),PY(cut[j][1]));',
'    x.lineTo(PX(bB[0]),PY(bB[1]));x.lineTo(PX(bA[0]),PY(bA[1]));x.closePath();',
'    x.fillStyle="rgba(180,130,70,0.30)";x.fill();x.strokeStyle="#b07c3e";x.lineWidth=2;x.stroke();',
'    x.fillStyle="#8a8a8a";x.font="10px Arial";x.textAlign="left";x.fillText("Ground",PX(bA[0])-12,PY(bA[1])-5);',
'    x.textAlign="right";x.fillText("Top",PX(M.run)+40,PY(M.rise)-5);',
'    dim(x,PX(bA[0]),PY(bA[1]),PX(bB[0]),PY(bB[1]),COL.H); tag(x,PX(M.run/2-nx*M.bwidth)+8,PY(M.rise/2-ny*M.bwidth)+10,"H",COL.H);',
'    dim(x,PX(M.run),PY(M.rise),PX(bB[0]),PY(bB[1]),COL.G); tag(x,PX(M.run-nx*M.bwidth/2)+8,PY(M.rise-ny*M.bwidth/2),"G",COL.G);',
'    if(M.treads>=2){ dim(x,PX(M.tread)+2,PY(M.riser),PX(M.tread)+2,PY(2*M.riser),COL.C); tag(x,PX(M.tread)+11,PY(1.5*M.riser),"C",COL.C); }',
'    dim(x,PX(0),PY(M.riser)-8,PX(M.tread),PY(M.riser)-8,COL.D); tag(x,PX(M.tread/2),PY(M.riser)-17,"D",COL.D);',
'    dim(x,PX(0)-9,PY(0),PX(0)-9,PY(M.first),COL.F); tag(x,PX(0)-18,PY(M.first/2),"F",COL.F);',
'    var inX=0-nx*M.throat, inY=0-ny*M.throat; dim(x,PX(0),PY(0),PX(inX),PY(inY),COL.I); tag(x,PX(inX/2)+9,PY(inY/2)+4,"I",COL.I);',
'  }',
'  function postResult(){',
