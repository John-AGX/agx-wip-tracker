...
//      morning (07:00–22:00 server-local) and deduped per (user, day) so
//      everyone gets a single "here's your day" nudge, not one email per
//      task and not one at midnight.
//
...
//      once (deduped per event+start so re-ticks don't re-send).
//
...
async function gatherTaskDigests() {
  var sql = [
    ...
    "       t.id, t.title, t.priority, t.due_date, t.entity_type,",
    '       (t.due_date - CURRENT_DATE) AS days_off',
    'FROM tasks t',
  ...
  r.rows.forEach(function (row) {
    ...
    if (!byUser[k]) {
      byUser[k] = { uid: row.uid, email: row.email, name: row.name, prefs: row.notification_prefs || {}, tasks: [] };
    }
...
function buildTaskDigestEmail(user, tasks) {
  var base = appUrl();
  var hostLabel = base.replace(/^https?:\/\//, '');
  var overdue = tasks.filter(function (t) { return Number(t.days_off) < 0; });
  var dueToday = tasks.filter(function (t) { return Number(t.days_off) === 0; });
  ...
  function rowHtml(t) {
    ...
    var prChip = pr
      ...
      : '';
    var due = fmtDueLabel(t.due_date);
    return '<tr><td style="padding:6px 10px;border-bottom:1px solid #eef0f3;font-size:14px;">' +
      escHtml(t.title || '(untitled task)') + prChip +
      '<span style="display:block;font-size:11px;color:#6b7280;margin-top:1px;">Due ' + escHtml(due) + '</span>' +
      '</td></tr>';
  }
  ...
  var text =
    'Your tasks for today (' + total + ')\n\n' +
    (overdue.length ? 'OVERDUE (' + overdue.length + '):\n' + overdue.map(function (t) { return '  - ' + (t.title || '(untitled)') + ' (due ' + fmtDueLabel(t.due_date) + ')'; }).join('\n') + '\n\n' : '') +
...
async function gatherEventReminders() {
  // Events whose start is now within reminder_minutes (i.e., it's time to
  // remind) and not yet started. Owner must be active with an email.
  var sql = [
...
function buildEventReminderEmail(ev) {
  var base = appUrl();
  var hostLabel = base.replace(/^https?:\/\//, '');
  var when = ev.all_day ? (fmtDueLabel(ev.starts_at) + ' (all day)') : fmtEventTime(ev.starts_at);
...
async function runOnce(opts) {
  ...
  try {
    ...
    var digestWindow = hour >= 7 && hour < 22;
    out.digestWindow = digestWindow;
    var dirty = false;

    // 1) Task-due digests (one per user). Morning-gated for real sends;
    //    dry runs always report candidates.
    var byUser = await gatherTaskDigests();
    var userKeys = Object.keys(byUser);
    out.tasks.candidates = userKeys.length;
    if (digestWindow || dry) {
      for (var i = 0; i < userKeys.length; i++) {
        var u = byUser[userKeys[i]];
        if (u.prefs && u.prefs.task_due === false) { out.tasks.skipped++; continue; }
        var dkey = 'taskdigest|' + u.uid + '|' + today;
        if (fires[dkey]) { out.tasks.skipped++; continue; }
        if (dry) { out.tasks.sent++; continue; } // would-send count
        try {
          var de = buildTaskDigestEmail(u, u.tasks);
    ...
    // 2) Event reminders (one per event).
    ...
    for (var j = 0; j < events.length; j++) {
      ...
      try {
        var ee = buildEventReminderEmail(ev);
