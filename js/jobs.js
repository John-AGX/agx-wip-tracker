...
function renderJobsMain() {
        ...
        window.p86ScopeViewSet = function (btn, view) {
            try {
                var panel = btn.closest('.p86-scope-panel'); if (!panel) return;
                panel.querySelectorAll('.p86-sc-seg button').forEach(function (b) { b.classList.toggle('on', b.getAttribute('data-scv') === view); });
                var lw = panel.querySelector('.p86-sc-list-wrap'); var mw = panel.querySelector('.p86-sc-matrix-wrap');
                if (lw) lw.style.display = (view === 'matrix') ? 'none' : '';
                if (mw) mw.style.display = (view === 'matrix') ? '' : 'none';
            } catch (e) {}
        };

        // RE-LIT (scope rework): the "Scope Manager" look — a compact stat strip,
        // a List / Matrix toggle (defaults to List), and clean money-line-style
        // scope rows (chips + hero revenue + progress) unified with the Buildings
        // cards. The busy Scopes × Buildings allocation matrix moves BEHIND the
        // Matrix toggle. Written fresh per John's decision; data model stays
        ...
        function renderOverviewPhasesInto(container, jobId, phases) {
            ...
            var stripHTML;
            if (_recon.contract > 0) {
                var _allocCls = _recon.full ? 'good' : (_recon.over ? 'warn' : '');
                var _gapCls = (_recon.gap > 0.5) ? 'warn' : '';
                stripHTML =
                ...
                stripHTML =
                    '<div class="p86-sc-strip">' +
                        '<div class="st"><div class="k">Revenue</div><div class="v good">' + formatCurrency(totalRev) + '</div></div>' +
                        '<div class="st"><div class="k">Cost</div><div class="v">' + formatCurrency(totalCost) + '</div></div>' +
                        '<div class="st"><div class="k">Profit</div><div class="v ' + (totalProfit >= 0 ? 'good' : 'warn') + '">' + formatCurrency(totalProfit) + '</div></div>' +
                        '<div class="st"><div class="k">Avg %</div><div class="v">' + jobPct + '%</div></div>' +
                    '</div>';
            }
            ...
            var _showMatrix = _bldgs.length > 0 && !_suppressMatrix;

            var headHTML =
                '<div class="p86-sc-head">' +
            ...
            // Clean money-line scope rows: name + chips, hero revenue, progress.
            var rowsHTML = groupKeys.map(function (phaseName) {
                ...
                return '<div class="p86-sc-row">' +
                    ...
                    '<div class="p86-sc-btm">' +
                        ...
                        '<div class="p86-sc-bar"><i style="width:' + Math.max(0, Math.min(100, avgPct)) + '%"></i></div>' +
                        '<span class="p86-sc-pct">' + avgPct + '%</span>' +
                    '</div>' +
                '</div>';
            }).join('');
