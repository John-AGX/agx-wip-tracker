...
(function () {
    ...
    pane.innerHTML =
      '<div style="max-width:1100px;margin:0 auto;padding:8px 4px 40px;">' +
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">' +
          '<h2 style="margin:0;font-size:20px;font-weight:700;flex:1;">📐 Plans &amp; Drawings</h2>' +
          '<button class="primary" id="p86-plans-new" style="padding:8px 16px;border-radius:8px;font-weight:600;cursor:pointer;">➕ New Drawing</button>' +
        '</div>' +
        '<div style="font-size:12.5px;color:var(--text-dim,#9aa);margin-bottom:18px;line-height:1.5;">' +
          'A CAD-style drafting workspace — draw shop drawings to scale with a titleblock, plan + elevation views, ortho snapping, dimensions, layers, and print-ready PDF / PNG output. ' +
          'Marking up a <strong>photo</strong> or <strong>PDF</strong> instead? Open it from <strong>Files</strong> and use the ✏️ annotate / 📐 Measure button.' +
        '</div>' +
  ...
  function openCreate() {
    ...
    box.innerHTML =
      ...
      '<input id="p86-plan-name" type="text" autocomplete="off" value="Untitled drawing" ' +
    ...
    function commit() {
      var name = (input.value || '').trim() || 'Untitled drawing';
      close();
      // Plans are always CAD drafting sheets now. (Photo/PDF takeoff
      // lives in the markup viewer, launched from Files.)
      api().plans.create({ name: name, base_kind: 'sheet', pages: [], totals: {} }).then(function (resp) {
