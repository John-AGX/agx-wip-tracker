...
(function () {
  ...
  function shellHTML() {
    return '<div class="p86-wob">' +
      ...
      '<div class="p86-wob-scroll">' +
        '<div class="p86-wob-colhead" aria-hidden="true">' +
          ['', 'Ticket', 'Status', 'Scheduled', 'Due', 'Assignee', 'Buildings', 'Crew'].map(function (h) {
  ...
  function statusPillsHTML() {
    return STATUS_PILLS.map(function (p) {
      var on = p.id === _state.status;
      var n = countOf(_state.statusCounts, p.group);
      return '<button type="button" class="p86-st-pill' + (on ? ' active' : '') + '" data-status="' + esc(p.id) + '"' +
  ...
  function viewPillsHTML() {
    return VIEWS.map(function (v) {
      ...
      var n = countOf(_state.counts, v.id);
  ...
  function rowHTML(r) {
    ...
    return '<a class="p86-wob-row" href="' + esc(hrefFor(r)) + '" data-id="' + esc(r.id) + '" aria-label="' + esc(label) + '">' +
      '<span class="p86-wob-cell p86-wob-c-prio"><span class="p86-wob-k">Priority</span><span class="p86-st-prio prio-' + esc(prio) + '" title="' + esc(PRIORITY_LABEL[prio] + ' priority') + '"></span></span>' +
  ...
  function listHTML() {
    ...
    return _state.rows.map(rowHTML).join('');
  }
  ...
  function focusKey() {
    ...
    var attrs = ['data-status', 'data-view', 'data-id'];
  ...
  function load(mode) {
    ...
    var params = paramsFor(limit, offset);
    if (mode === 'reset') {
      _state.loading = true;
      _state.error = false;
      _state.rows = [];
    ...
    var p;
    try { p = Promise.resolve(a.list(params)); } catch (e) { p = Promise.reject(e); }
    return p.then(function (res) {
      if (my !== _state.seq) return;
      var body = res || {};
      var rows = Array.isArray(body.tickets) ? body.tickets.map(pick) : [];
