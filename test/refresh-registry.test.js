...
describe('mutation site → registry: nothing patches a money store behind the registry', () => {
  ...
  const ALLOWED = {
    ...
    'jobs.js': {
      ...
      loadPurchaseOrdersForJob: { sites: 3, in: ['<top-level>', 'renderJobChangeOrdersInto', 'renderJobPurchaseOrdersInto'], why: 'DEFINES it — the registry `po` store resolves this very function off window — plus TWO first-paint reads. renderJobPurchaseOrdersInto is the older one, filling the accrued tile. renderJobChangeOrdersInto is the newer: the change-order list now carries a "Cost draws on" column, and appData.jobPurchaseOrders loads per job on demand, so without this read a change order bound to a perfectly good purchase order would render as "re-bind" — a wrong state, shown confidently, on a money surface. Paired with loadChangeOrdersForJob in one Promise.all so the list paints once both reads land. A READ on a view transition; the write path (openCoAllocEditor save) goes through p86Refresh.now.' },
      ...
      'loadPurchaseOrdersForJob (no force)': { sites: 2, in: ['renderJobChangeOrdersInto', 'renderJobPurchaseOrdersInto'], why: 'Two tab-open paths, both unforced for the same reason. renderJobPurchaseOrdersInto pairs it with loadBillsForJob in one Promise.all so the money tab paints once both reads land; renderJobChangeOrdersInto pairs it with loadChangeOrdersForJob so the "Cost draws on" column has purchase orders to judge against before it judges. Opening a tab is navigation, not a write, so joining an in-flight GET is the point — forcing would issue a duplicate fetch on every tab switch, and no write preceded either call, so there is no pre-write GET to be fooled by.' },
