...
describe('REGISTER 2 — the route population', () => {
  ...
  test('the ROUTE count is committed (615 across 79 routers)', () => {
    ...
    //   587 -> 594, +7 ALL WAIVED, guest PROPOSE and internal PARTICIPANTS (S6).
    //      Six owner doors and ONE public door; every one takes a path
    //      parameter, so the driven count does not move:
    //        GET    /api/service-tickets/:id/revisions
    //        POST   /api/service-tickets/:id/revisions/:rid/accept
    //        POST   /api/service-tickets/:id/revisions/:rid/reject
    //        GET    /api/service-tickets/:id/participants
    //        POST   /api/service-tickets/:id/participants
    //        DELETE /api/service-tickets/:id/participants/:userId
    //        POST   /api/service-ticket-share/:token/revision   <- public door
    //      The public door is the answer to "let the guest edit": it writes a
    //      PROPOSAL into service_ticket_revisions and never touches
    //      service_tickets, so the widest scope a token can hold still cannot
    //      change the work order. status, organization_id and assignee are
    //      dropped at emit AND again at apply, because a row written by an
    //      older or compromised path must not be able to widen what it sets.
    //      The accept re-read is pinned to status = 'pending' under FOR UPDATE,
    //      so a replay is a 404 by predicate rather than by a flag somebody has
    //      to remember to check.
    //      The two GETs are waived on the path parameter, not on being reads:
    //      both load the ticket through loadOwnedTicket and carry
    //      organization_id in the WHERE.
    //      The participants routes mint NO TOKEN — that is the reason they are
    //      a separate table from service_ticket_shares. A token for an employee
    //      would bypass their own role, survive their deactivation, and be
    //      forwardable outside the company.
    //   594 -> 595, +1 WAIVED, the delete-impact preview (2639e755):
    //        POST /api/leads/delete-impact
    //      Waived because it is a POST. It reads rather than writes — it
    //      answers "what would this delete take with it" so the confirm can
    //      say so — and it is a POST only because it takes a list of ids.
    //   595 -> 596, +1 WAIVED, the one-line card's shown report (fast approval):
    ...
    //   601 -> 603, +2 ALL WAIVED, the work-order "Fill from a job file" doors
    //      (John, 2026-09-13). Both take :id, so the driven count does not move:
    //        GET  /api/service-tickets/:id/materials/sources
    //        POST /api/service-tickets/:id/materials/extract
    //      Neither writes. Both load the ticket through loadOwnedTicket
    //      (`organization_id = $2`) and ask for WRITE access on it, because
    //      the list only exists to fill an editor a viewer cannot save. The
    //      files are keyed on the ticket's PARENTS, each proved first: the job
    //      by the ticket's own proof, the lead and the estimate by
    //      `id = $1 AND organization_id = $2` plus LEADS_VIEW / ESTIMATES_VIEW,
    //      so the narrow tier cannot read a sales lead's or a priced estimate's
    //      files through a work order. The extract door puts those parent
    ...
    // 626 -> 627: GET /api/weather/leads, on the EXISTING weather router — no
    //      new router. Today's per-site conditions for the Leads map. It reads
    //      leads through `(organization_id = $2 OR organization_id IS NULL)`,
    //      the same predicate GET /api/map/entities uses for leads, and an id
    //      from another organization answers 'unknown_lead' with no coordinates
