...
var worker_default = {
  async email(message, env2, ctx) {
    ...
    const mailbox = /* @__PURE__ */ __name((a) => a && a.address ? a : a && Array.isArray(a.group) && a.group[0] || null, "mailbox");
    const addrStr = /* @__PURE__ */ __name((a) => {
      const m = mailbox(a);
      return m ? m.name ? `${m.name} <${m.address}>` : m.address : "";
    }, "addrStr");
    const addrList = /* @__PURE__ */ __name((arr) => (arr || []).map(addrStr).filter(Boolean), "addrList");
    const PER_FILE = 18 * 1024 * 1024;
    const TOTAL_CAP = 22 * 1024 * 1024;
    const toBase64 = /* @__PURE__ */ __name((u8) => {
      let out = "";
      const CHUNK = 32768;
    ...
    let resp;
    try {
      resp = await fetch(env2.P86_INBOUND_URL, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-P86-Inbound-Secret": env2.P86_INBOUND_SECRET || ""
        },
