...
1. **Stop the bleeding** — CO/PO/invoice payload ops writing to dead arrays (§3, data loss) → then `organization_id` stamping + the client‑directory org predicate (§3, tenancy).
2. **Fix the prompt lie** about `emit_payload_file` + cut `<available_tools>` (§5, §6).
3. **Port the money layer** to `server/services/money/` with fixture tests (§4) — unblocks everything downstream.
