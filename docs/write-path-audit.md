...
1. ~~**Stop the bleeding** — CO/PO/invoice payload ops writing to dead arrays (§3, data loss) → then `organization_id` stamping + the client‑directory org predicate (§3, tenancy).~~ **✅ SHIPPED `bf16ead`.** `server/services/job-financials.js` is now the one write layer for COs/POs/invoices, taking an explicit `db` so the dispatcher hands it the open transaction; it stamps `organization_id` on POs and resolves every write through the job's org. The REST routes import its helpers. **The client‑directory org predicate is still open.**
   ...
   - ~~**Still open (deliberately unbundled):** four of 86's client‑side tools…~~ **✅ RETIRED.** It was **five**, not four — `set_invoice_field` was missed. All five schemas are gone from `JOB_TOOLS` and all five executor cases from `js/ai-panel.js`. **Correction to the earlier note: they were never live.** `ROUTER_TOOL_NAMES` (`admin-agents-routes.js:2501`, enforced at `:2560`) filtered them out before managed‑agent registration, so 86 could not call them and **no resync was needed** — they were unreachable code advertising a capability the payload path now really provides.

   **The second sweep (64 confirmed findings, 70 agents) found the same class in five more places.** Fixed alongside:
   - `read_wip_summary` still read the dead blob, so the **company‑wide** WIP rollup booked $0 of CO revenue — and once `buildJobContext` was fixed, the two AI surfaces contradicted each other inside one conversation. Now batched over the real tables (one query for the org, not one per job).
   - `computeJobWIP` **ignored its `jobInvoices` argument entirely** and read the hand‑typed `job.invoicedToDate` scalar, so the WIP block reported `$0 invoiced` while the `# Invoices` block in the same snapshot listed real AR invoices.
   - The `emit_payload_file` description told the model a CO takes `income` + `estimated_costs`. Nothing reads those keys — **every agent‑created CO would have landed at $0.** Description rewritten to the real line‑item shape, and `rejectFlatMoney` now *bounces* the wrong shape rather than saving a $0 record (a bounced tool call teaches the model; a fabricated line item to make the number come out right does not).
   - Job‑scoped CO/PO update/delete ops were scoped by **org only, never `job_id`** — a payload for job A could reach job B's records.
   - The building modal's **"+ Add CO"** wrote a flat‑shaped CO into `appData.changeOrders` (and used native `prompt()`, a no‑op in the installed PWA). It now points at the Change Orders tab. **Building‑level allocation of a real CO is a genuine feature port and is NOT implemented** — it needs the CO editor's line model.
...
3. **Port the money layer** to `server/services/money/` with fixture tests (§4) — unblocks everything downstream. **Started in `bf16ead`:** `js/pricing-pipeline.js` is now dual‑target (`window.p86Pricing` in the browser, `module.exports` on the server) and `money/change-order-totals.js` derives CO income/costs from it, so server and client totals cannot drift by construction. Remaining: estimate totals, WIP, and the phase/revenue rollup.
   - **✅ `getJobWIP` PORTED @b2fad44** → `server/services/money/job-wip.js`; the drifted in‑file `computeJobWIP` is deleted. It had *claimed* to mirror the browser but omitted linked QB actuals, vendor bills, PO/sub accrual, projected cost, and `displayProfit`/`displayMargin` — so 86 quoted job margin from a formula no screen uses, while the `read_wip_summary` description told it the numbers matched the WIP tiles. Both call sites now use batched loaders (`loadWipInputs`), and 15 differential assertions pin it to hand‑computed browser math.
     - **Remaining Tier‑B candidate: `computeEstimateTotals`** (`js/estimates.js:29` — the only blocker is the `appData.estimateLines` global). Its `proposalTotal` becomes the job's `contractAmount` at conversion, stored verbatim with no server re‑derivation.
     - **Tier‑C, do not attempt:** the node‑graph rollup (`nodegraph/ui.js` `pushToJob`) and `deriveSOV` — both depend on stateful graph walks (`E.getOutput`/`E.resetComp`, `NG.*` queries), not portable math.
4. **Critic + deterministic hooks at `applyPayload`** with commit‑time re‑derive/diff + `tool_use_id` idempotency. **Double‑apply is closed:** both apply doors (`POST /payloads/:id/apply` and the approve‑in‑chat `applyPayloadForUser`) now take an atomic `status='applying'` claim, so a double‑click, a retry, or a replayed tool call can no longer create every record in a payload twice — the dispatcher's advisory locks serialized those two applies but never collapsed them. A boot‑time reset in `db.js` releases claims stranded by a dead process. The critic itself is still open.
   - **Tenancy closed in the same pass:** `dispatchClient` / `dispatchEstimate` / `dispatchLead` / `dispatchSchedule` creates and `execProposeCreateLead` all stamped `organization_id` NULL. Because every org‑scoped read carries `OR organization_id IS NULL` to keep pre‑tenancy rows visible, an AI‑created client, lead, estimate, or schedule entry was readable **and writable by every tenant**. All now stamp from `ctx`, never from model‑supplied fields.
   - **All 14 gating client lookups** in `execClientDirectoryTool` were unscoped — `delete_client` was the sharpest, deleting another tenant's client outright. Every one is now org‑scoped and the writers fail closed without a resolved org.
   - `dispatchEstimate` update never checked `is_locked`, so a payload could rewrite a **sold** estimate after its total had become the job's contract amount.
5. **Service extraction per entity** using the `dispatchAssembly` template; then **generate** the op grammar + tool description from the services.
   - 🔴 **NEW, pre‑existing, unfixed: the `emit_payload_file` description is 4,680 chars against a hard 1,024‑char cap** (`toCustomToolParam`, `admin-agents-routes.js:2173‑2183`, comment says "the agent still sees the first 1024 chars which is plenty to disambiguate"). **78% of the payload grammar has never reached the model through the tool schema** — everything from `report:` onward, including all four calendar/reminder/task/todo types and every TARGET FORM (conditional / bulk / move). The Scribe recovers most of it from its system baseline; whether **86** does is unverified. The CO/PO/invoice money contract was moved into that baseline and a one‑line pointer left in the description, which is the pattern the rest should follow. Generating the grammar from the services (this item) is the real fix — the description should shrink to a pointer.
