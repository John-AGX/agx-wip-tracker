...
| G4 | Injection ordering: dedup/today-digest mutate `turnContextText` **after** `turnText` is frozen (`:13754/:13834`) — a naive hook would inject into a message already assembled | hook digest injection before turn-text assembly; fix the dedup scope/order in the same pass | M |
